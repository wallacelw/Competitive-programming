...
struct SparseTable {
    ...
    ll n, m;
    vector<vector<ll>> st; // st[j][i] covers range [i, i+2^j)

    // 0-idx: [0, n)
    SparseTable(vector<ll> &v) : n(v.size()), st({v}) {
        m = MSB(n);
    ...
    ll query(ll l, ll r) { // query for [l, r] in O(1)
    ...
    ll query(ll l, ll r) { // query for [l, r] in O(log)
        ll ans = 0; // define the correct default null value here
        for(ll k=m; k>=0; k--) {
            if ((1LL << k) <= (r-l+1)) {
                ans = f(ans, st[k][l]);
                l += 1LL << k;
